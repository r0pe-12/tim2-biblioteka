                                            ...
                                            <tbody>
                                                ...
                                                <tr>
                                                    <td align="center" style="font-size:18px;color:#0e0e0f;font-weight:700;font-family:Verdana;line-height:28px;vertical-align:top;text-align:center;padding:35px 40px 0px 40px">
                                                        <strong>Knjiga uspjesno {{ strtolower($reservation->status()->name) }}</strong>
                                                        <p style="font-size: 15px; margin-bottom: 0">ID - {{ $reservation->id }}</p>
                                                    </td>
                                                </tr>
                                                ...
                                                <tr>
                                                    <td class="m_2113659865872709336content" style="font:16px/22px 'Helvetica Neue',Arial,'sans-serif';text-align:left;color:#555555;padding:40px 40px 0 40px">
                                                        <p> Zdravo {{ $reservation->student->name }}, </p>
                                                        <p> Ovo je potvrda da je bibliotekar "{{ $reservation->librarian->name }} {{ $reservation->librarian->surname }}" odobrio rezervaciju knjige <b>"{{ $reservation->book->title }}"</b> dana {{ \App\Models\Carbon::parse($reservation->status()->datum)->format('d.m.Y.') }} </p>
                                                        <p> Rezervacija istice: <b>{{ App\Models\Carbon::parse($reservation->submttingDate)->addDays($res_deadline->value)->format('d.m.Y') }}</b> </p>
