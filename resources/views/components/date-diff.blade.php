...
@else
    @if($failed == "true")
        {{ $zapis->return_date < today('Europe/Belgrade') ? str_replace(['pre', 'nedelju', 'mesec', '1 sekundu'], ['', 'nedelja', 'mjesec', 'Manje od jednog dana'], \App\Models\Carbon::parse($zapis->return_date)->diffInDays(date('Y-m-d', strtotime($zapis->statuses()->latest()->first()->pivot->datum)))) . ' dana' : 'Nema prekoracenja' }}
    @else
